...
func TestVideoTrimIntegrationClipAndRemove(t *testing.T) {
	...
	if err := runTrimFFmpeg(input, clipOut, "1", "", "2", "mp4", "reencode", 70, converter.MetadataAuto, false, nil); err != nil {
		t.Fatalf("runTrimFFmpeg failed: %v", err)
	}
	assertFileHasContent(t, clipOut)

	if err := runTrimRemoveFFmpeg(input, removeOut, "1", "", "2", "mp4", "reencode", 70, converter.MetadataAuto, false, nil); err != nil {
